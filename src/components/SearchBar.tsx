...
import { ClearDictionaryResults, DictionaryQuery } from "../store/dictionary.store";
    ...
    return (
        <div className="flex items-center justify-between bg-neutral-100 rounded-md">
            <input 
                type="text"
                value={model}
                className="p-2 w-full bg-neutral-100 rounded-md focus:outline-none"
                name="dctnry-search-bar"
                id="dctnry-search-bar"
                onChange={handleInputChange}
            />
            <button type="button" className="pr-2 focus:outline-none" onClick={removeQuery}>
                {
                    model === "" ? 
                        <svg
